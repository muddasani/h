...
class TestConfirmRedirectUri(object):
    def test_returns_true_for_matching_redirect_uris(self, svc, client, authz_code):
        result = svc.confirm_redirect_uri(client.client_id, authz_code.code, authz_code.redirect_uri, client)
        assert result is True

    def test_falls_back_to_client_redirect_uri(self, svc, client, authz_code):
        result = svc.confirm_redirect_uri(client.client_id, authz_code.code, None, client)
        assert result is True

    def test_returns_false_when_redirect_uris_do_not_match(self, svc, client, authz_code):
        result = svc.confirm_redirect_uri(client.client_id, authz_code.code, 'invalid', client)
...
class TestValidateCode(object):
    def test_returns_true_for_correct_code(self, svc, authz_code, client):
